...
const { bank } = require('../US-API/bank');

exports.list_all_bankAccounts = function (req, res) {
  const param = {};

  bank.list(param, function (err, bank_accounts) {
  ...
  stripe.tokens.create(
    ...
    function (err, token) {
      ...
      if (err) {
        ...
        stripe.customers.createSource(userID, param, function (err, bank_account) {
          // asynchronously called
          if (err) {
            console.log(err);
            res('REQUEST ERROR');
...
exports.read_a_bankAccount = function (req, res) {
  ...
  stripe.customers.retrieveCard(userID, bankAccountID, function (err, external_account) {
    // asynchronously called
    if (err) {
      console.log(err);
      res('REQUEST ERROR');
...
exports.update_a_bankAccount = function (req, res) {
  ...
  stripe.customers.updateCard(userID, bankAccountID, param, function (err, bank_account) {
    // asynchronously called
    if (err) {
      console.log(err);
      res('REQUEST ERROR');
...
exports.delete_a_bankAccount = function (req, res) {
  ...
  stripe.customers.deleteSource(userID, bankAccountID, function (err, confirmation) {
    // asynchronously called
    if (err) {
      console.log(err);
      res('REQUEST ERROR');
